...
for index, row in complete.iterrows():
    try:
        ...
        result1 = subprocess.run(["cargo", "new", f"{cve_id}_repair", "--edition", str(edition)], capture_output=True, text=True)
        if result1.returncode == 0:
            ...
            continue
        result2 = subprocess.run(["cargo", "new", f"{cve_id}_bug", "--edition", str(edition)], capture_output=True, text=True)
        if result2.returncode == 0:
            print(f"✅ Project {cve_id}_bug created successfully.")
        else:
            print(f"❌ Failed to create Rust project {cve_id}.")
            print("Error message:", result2.stderr)
        with open(f"./{cve_id}_repair/rust-toolchain.toml", "w") as f1:
            f1.write(f'''[toolchain]\nchannel = "{row["std_version"]}"''')
            f1.flush()
            f1.close()
        with open(f"./{cve_id}_bug/rust-toolchain.toml", "w") as f2:
            f2.write(f'''[toolchain]\nchannel = "{row["std_version"]}"''')
            f2.flush()
            f2.close()
        with open(f"./{cve_id}_repair/Cargo.toml", "a") as f3:
            f3.write(f'''{name} = {{git = "{repo_url}", rev = "{repair_hash}"}}''')
            f3.flush()
            f3.close()
        with open(f"./{cve_id}_bug/Cargo.toml", "a") as f4:
        ...
        if rust_code is not None:
            with open(f"./{cve_id}_repair/src/main.rs", "w") as f5:
                f5.write(rust_code)
                f5.flush()
                f5.close()
            with open(f"./{cve_id}_bug/src/main.rs", "w") as f6:
                f6.write(rust_code)
                f6.flush()
                f6.close()    
        print(f"✅ Project {cve_id} 填充完成.")
    except Exception as e:
        path1 = f"./{cve_id}_repair/"
        path2 = f"./{cve_id}_bug/"        
